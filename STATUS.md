...
### LoCoMo budget candidate-stage diagnosis (2026-09-22)

The text-only first-sample pilot now records candidate-stage evidence coverage. With the same source SHA-256 and 131 eligible questions, official evidence reached the budget pool in **36/131** questions. Of **43** questions where full-corpus BM25 hit evidence but budget missed, **42** lost all evidence before ranking and **1** retained evidence in the pool but did not rank it into Top-5. A deterministic public first-question repro stays green at 100 dialogue turns and turns red at 110: the FTS AND query has zero matches; its OR fallback has 103 matches, and recency-ordered `rowid DESC` puts the evidence at rank 101 behind the 100-candidate cap. FTS BM25 ordering would rank that evidence first **for this query only**. This diagnoses the candidate boundary, not a safe production replacement or an all-sample quality gain. An experiment-only diagnostic and one synthetic regression test were added; optional-semantic-import-isolated full suite: **394 passed in 17.37s**. No SDK/MCP retrieval defaults or private human labels changed. See [protocol](experiments/SESSION_EVALUATION.md) and [aggregate report](results/locomo_text_dialogue_pilot.json).

### LoCoMo text-evidence pilot (2026-09-22)

The [official LoCoMo dataset](https://github.com/snap-research/locomo/blob/main/data/locomo10.json) was run through the existing external session evaluator using one dialogue turn per atom and official dialogue evidence IDs, without generated summaries, graph edges, image content, or answer generation. In the first sample (`conv-26`), 131/199 questions had present, text-only evidence; 65 image-evidence, 2 no-evidence, and 1 missing-ID questions were excluded. At Top-5, macro evidence recall was BM25 **0.445**, TF-IDF **0.368**, and current budget **0.176**. Input SHA-256, exclusions, and aggregate report are in [results/locomo_text_dialogue_pilot.json](results/locomo_text_dialogue_pilot.json); replay protocol is in [experiments/SESSION_EVALUATION.md](experiments/SESSION_EVALUATION.md). One adapter test and optional-semantic-import-isolated full regression: **393 passed in 17.62s**. This is one public sample's retrieval-only pilot, not a full LoCoMo QA score, independent product comparison, causal-graph quality result, or license to tune production defaults. The 96-row private human review remains unfilled.

### Source-traceable causal-direction probe (2026-09-22)

Three locally archived troubleshooting summaries were paraphrased into a public 12-atom/3-question [fixture](experiments/causal_source_cases.json), with separate cause, symptom, and remedy atoms and cause → symptom edges. The local Wiki retains the exact source-to-case mapping; the public [anonymous report](results/causal_source_cases_probe.json) contains no original atom IDs or source text. In TF-IDF precision Top-5, baseline and the experimental directional chain each retrieve **2/3** assistant-labeled causes; the existing undirected bridge retrieves **3/3**. All three directional cases stop at `no_primary_outgoing_candidate`, and all three questions return at least one assistant-labeled negative under every strategy. These are source-traceable but assistant-interpreted examples, not independent causal or relevance judgments, and no timing was measured. Two new report-boundary tests; optional-semantic-import-isolated full regression: **389 passed in 16.11s**. Production defaults and the unfilled 96-row human review remain unchanged.
