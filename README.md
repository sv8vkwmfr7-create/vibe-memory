...
该脚本使用内存 SQLite、TF-IDF、关闭自动建边和 Episode 聚合，以隔离 SDK 核心写入与 `recall()` 路径；每次写入计时，并对均匀采样的刚写入 atom 直接调用 `storage.get_atom()`。TF-IDF 使用稀疏倒排索引，BM25 只访问查询词 posting；`budget` 模式先按租户、生命周期和查询词在存储层收窄到至多 `max(100, top_k * 20)` 个候选。20 个确定性查询用于分别测量首次冷查询和后续热查询。Windows + Python 3.12.14 的一次串行运行结果：

| 记忆量 | 写入 ops/s | 写入 p50/p95/p99 (ms) | 写后读取 p50/p95/p99 (ms) | recall p50/p95/p99 (ms) | 可见性 |
|-------:|-----------:|----------------------:|--------------------------:|------------------------:|:-------|
| 1,000 | 19,902.91 | 0.043 / 0.056 / 0.116 | 0.011 / 0.019 / 0.025 | 3.148 / 4.094 / 6.477 | 280/280 (100%) |
| 10,000 | 20,556.02 | 0.045 / 0.058 / 0.101 | 0.012 / 0.061 / 0.067 | 18.893 / 22.176 / 22.337 | 298/298 (100%) |
| 100,000 | 19,053.54 | 0.047 / 0.089 / 0.132 | 0.012 / 0.070 / 0.075 | 182.702 / 213.454 / 221.227 | 299/299 (100%) |

| 记忆量 | cold recall (ms) | warm recall p50/p95/p99 (ms) |
|-------:|-----------------:|-----------------------------:|
| 1,000 | 7.072 | 3.137 / 3.520 / 3.854 |
| 10,000 | 21.374 | 18.878 / 22.187 / 22.340 |
| 100,000 | 188.559 | 182.664 / 213.965 / 221.329 |

这是单进程、内存数据库且关闭建边/聚合的可复现回归基线；“可见性”指提交后直接 `storage.get_atom()` 可读，不等于异步索引完成或新记忆立即出现在 `recall()` 结果中。加入 `budget` 存储层候选查询后，同一 v2 口径相对上一轮 1k/10k/100k 的 warm p95 分别改善约 64.8%/81.1%/85.7%，100k cold recall 为 189 ms。当前 SQL 候选查询仍会扫描租户行并计算 `LIKE` 匹配；下一步应使用 SQLite FTS5 或持久化倒排索引，并补齐真正的写后检索可见性与并发压测。
